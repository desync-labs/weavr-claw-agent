...
export async function doctor(opts, deps = {}) {
  ...
  // The policy file, and which shipped preset it is: a re-run of init
  // without --policy keeps the file (edits and all) and rewrites only its
  // notice, so a fix that says to run init names the preset when the file
  // is one, and says the edits are kept when it is not.
  let policy = null;
  let preset = null;
  try {
    ...
    report.cross('policy file', `${paths.policyFile}: ${e.code === 'ENOENT' ? 'missing' : 'not valid JSON'}`, { fix: e.code === 'ENOENT' ? 'run weavr-curator init again; it writes the preset' : `fix the file, or move it aside and run weavr-curator init again (it writes the standard preset), or pass --policy <preset> --yes to write that preset over it` });
  }
  ...
  if (!exists(paths.composeEnv, fs)) {
    ...
    // Compose interpolates the process environment over --env-file. A leftover
    // CURATOR_KEY_FILE from another book mounts that book's key.
    for (const [k, pathKey] of Object.entries(COMPOSE_HOME_PATH_OF)) {
      const shell = String(env[k] ?? '').trim();
      if (!shell) continue;
      const expected = resolve(paths[pathKey]);
      const target = isAbsolute(shell) ? resolve(shell) : null;
      if (target === expected) continue;
      problems.push(`the shell ${k} is ${shell}, which overrides compose.env`);
    }
    if (problems.length) {
      const shellHit = problems.some((p) => p.startsWith('the shell '));
      report.cross('compose env', `${paths.composeEnv} ${problems.join('; ')}`, {
        fix: shellHit
          ? `unset ${COMPOSE_HOME_PATHS.join(' ')} before docker compose --env-file ${paths.composeEnv} -f curator/compose/curator.yml up -d`
          : `run weavr-curator init again with --home ${home}; it re-derives the chain facts and writes every variable curator.yml references, the path variables as the files and the agent home under the home`,
      });
    }
  ...
  if (row) {
    ...
    if (policy?.invariants) {
      ...
      if (want === have) report.ok('notice', `${have}s onchain and in policy.invariants.rebalanceDelaySecs`);
        ...
        // The init it tells the owner to run names the preset when the file
        // is one; a file that matches none holds the owner's edits, which a
        // re-run without --policy keeps, rewriting only the one number.
        const rewrite = preset
          ...
          : `weavr-curator init --portfolio ${mint} --home ${home} again, without --policy (it keeps ${paths.policyFile} as you edited it and rewrites only invariants.rebalanceDelaySecs to the chain's ${have}s)`;
  ...
  if (signerUp) {
    ...
    if (metrics.failure || metrics.status !== 200) {
      ...
      if (!('curator_last_tick_ts' in g)) {
        ...
        report.cross('agent heartbeat', others ? `/metrics answered but carries no curator_last_tick_ts gauge (${others} other curator gauges): an older signer image` : '/metrics answered but carries no curator gauges: not this signer, or an older image', { fix: 'pass --signer-url for the curator signer, or pull a current signer image (curator/compose/curator.yml names the published tag) and restart it' });
      } else if (!(lastTick > 0)) {
  ...
  if (docker.status !== 0) {
    ...
    // The signer image is published, so a tag that is not local yet is fine
    // when the registry serves it: compose pulls on `up`, and `docker
    // manifest inspect` asks without pulling. The agent image is local only.
    // A missing compose.env value falls back to what curator.yml names.
    const fallback = Object.fromEntries(composeVariables(fs.readFileSync(join(repoRoot, 'curator', 'compose', 'curator.yml'), 'utf8')).map((v) => [v.name, v.fallback]));
    const images = [['signer image', compose.CURATOR_SIGNER_IMAGE || fallback.CURATOR_SIGNER_IMAGE], ['agent image', compose.HERMES_IMAGE || fallback.HERMES_IMAGE]];
    for (const [name, image] of images) {
      const local = exec('docker', ['image', 'inspect', image]);
      if (local.status === 0) {
        report.ok(name, image);
        continue;
      }
      if (name === 'signer image') {
        const served = exec('docker', ['manifest', 'inspect', image]);
        if (served.status === 0) {
          report.ok(name, `${image}, not pulled yet (compose pulls it on up)`);
          continue;
        }
        report.cross(name, `${image} is not a local image and the registry does not serve it`, { fix: `docker pull ${image}, or set CURATOR_SIGNER_IMAGE in compose.env to a tag you trust (curator/compose/curator.yml names the published one)` });
        continue;
      }
      report.cross(name, `${image} is not a local image`, { fix: 'build the agent image from the Claw Agent checkout (docker compose build there), or set HERMES_IMAGE in compose.env' });
    }
    if (exists(paths.composeEnv, fs)) {
