...
export async function doctor(opts, deps = {}) {
  ...
  if (signerUp) {
    ...
    if (metrics.failure || metrics.status !== 200) {
      ...
      if (!('curator_last_tick_ts' in g)) {
        ...
        report.cross('agent heartbeat', others ? `/metrics answered but carries no curator_last_tick_ts gauge (${others} other curator gauges): an older signer image` : '/metrics answered but carries no curator gauges: not this signer, or an older image', { fix: 'pass --signer-url for the curator signer, or rebuild the signer image from the weavr backend dest branch and restart it' });
      } else if (!(lastTick > 0)) {
  ...
  if (docker.status !== 0) {
    ...
    for (const [name, image] of images) {
      ...
      else report.cross(name, `${image} is not a local image`, { fix: name === 'signer image' ? 'build it from the weavr backend dest branch: git clone -b dest <backend> && docker build -t weavr-backend:curator-local <backend> (main and demo have no curator package; or set CURATOR_SIGNER_IMAGE in compose.env)' : 'build the agent image from the Claw Agent checkout (docker compose build there), or set HERMES_IMAGE in compose.env' });
    }
    if (exists(paths.composeEnv, fs)) {
