...
/**
 * True when `doc` is `preset` apart from `_comment` keys and
 * `invariants.rebalanceDelaySecs`, the one number init rewrites from chain.
 * Anything else that differs is an edit of the owner's.
 */
export function samePresetApartFromNotice(doc, preset) {
  const norm = (d) => {
    const s = d && typeof d === 'object' && !Array.isArray(d) ? stripComments(d) : {};
    if (s.invariants && typeof s.invariants === 'object') s.invariants = { ...s.invariants, rebalanceDelaySecs: null };
    return canonicalJson(s);
  };
  return norm(doc) === norm(preset);
}

/**
 * The shipped preset `doc` was written from, or null when it matches none
 * (edited by hand). A re-run of init without --policy keeps this one, and
 * the doctor names it in the init it tells the owner to run.
 */
export function presetOf(doc, { repoRoot, fs = nodeFs } = {}) {
  return PRESETS.find((name) => samePresetApartFromNotice(doc, loadPreset(name, { repoRoot, fs }))) ?? null;
}
