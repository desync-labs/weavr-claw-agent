...
import { PRESETS, loadPreset, validatePolicyAgainstBook } from './policy-check.mjs';
...
export async function init(opts, deps = {}) {
  ...
  const policyName = opts.policy ?? 'standard';
  const apiUrl = opts.api ?? DEFAULT_API;
  const mcpUrl = opts.mcp ?? DEFAULT_MCP;
  ...
  if (!PRESETS.includes(policyName)) {
    report.cross('policy', `unknown preset ${JSON.stringify(policyName)}`, { fix: `pass --policy ${PRESETS.join(' or ')}` });
    return done(EXIT_INIT.USAGE);
  }
  ...
  try {
    ...
    const confirmSigning = async (who, what) => {
      ...
      if (!interactive || typeof prompt !== 'function') {
        report.cross('curation', 'stdin is not interactive, so the signing above cannot be confirmed here; nothing built, nothing signed', { fix: 'pass --yes to sign and send' });
        return false;
      }
    ...
    } else if (!interactive || typeof prompt !== 'function') {
      report.cross('confirm', 'stdin is not interactive, so the chain facts above cannot be confirmed here', { fix: 'pass --yes to accept them' });
      return done(EXIT_INIT.CROSS);
    } else {
    ...
    report.ok('policy', `the ${policyName} preset admits ${legsText(legs)}; written to ${paths.policyFile}`);
    ...
    const rpcUrl = (opts.rpc && String(opts.rpc)) || previousSignerEnv.SOLANA_RPC_URL || '';
    ...
    report.ok('tokens and env', `agent token ${signerTok.generated ? 'generated' : 'reused'}, ops token ${opsTok.generated ? 'generated' : 'reused'}; ${paths.signerEnv} (0600)${rpcUrl ? '' : ', SOLANA_RPC_URL left for you to set'}; ${paths.composeEnv}`, { rpcUrlSet: Boolean(rpcUrl) });
