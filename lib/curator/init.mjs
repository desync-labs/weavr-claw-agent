...
import { composeProjectName, copyProfile, providerFor, readConfigModel, renderAgentEnv, renderComposeEnv, renderSignerEnv } from './render.mjs';
...
export async function init(opts, deps = {}) {
  ...
  try {
    ...
    // The policy. --policy names a shipped preset and writes it. Without the
    // flag, a policy.json already in the home is the policy, the owner's
    // edits included: kept as it is, validated against the book exactly as
    // a preset is, and touched only to rewrite its notice from chain (a
    // rehearsal home stays on rehearsal across a re-run, even when the book
    // admits both). A first run without the flag writes standard. A file
    // that is not a JSON document can be neither kept nor checked.
    const previousPolicy = readPolicyFile(paths.policyFile, { repoRoot, fs });
    const explicitPreset = opts.policy !== undefined;
    const keepFile = !explicitPreset && previousPolicy.present;
    if (keepFile && previousPolicy.doc === null) {
      report.cross('policy file', `${paths.policyFile} is not a JSON document, so it can be neither kept nor checked`, { fix: `fix the file, or move it aside and run init again (it writes the standard preset), or pass --policy ${PRESETS.join('|')} --yes to write that preset over it` });
      return done(EXIT_INIT.CROSS);
    }
    // With --policy, a file that holds the owner's edits (or is not JSON) is
    // never written over silently: a cross names it before anything else is
    // written, and --yes is the only way past.
    if (explicitPreset && previousPolicy.present && previousPolicy.preset === null && !opts.yes) {
      const what = previousPolicy.doc === null ? 'is not a JSON document' : 'matches no shipped preset (edited by hand)';
      report.cross('policy file', `${paths.policyFile} ${what}; --policy ${opts.policy} would write the preset over it`, { fix: `move ${paths.policyFile} aside, or omit --policy to keep it (init checks it against the book and rewrites only its notice); or pass --yes with --policy to write the ${opts.policy} preset over it, then put your edits back` });
      return done(EXIT_INIT.CROSS);
    }
    const policyName = explicitPreset ? opts.policy : previousPolicy.preset ?? (keepFile ? null : 'standard');
    const policyFrom = explicitPreset ? 'flag' : keepFile ? 'home' : 'default';
    const policyDoc = keepFile ? previousPolicy.doc : loadPreset(policyName, { repoRoot, fs });
    // How the lines name the document: a preset by name; a kept file that
    // matches none by its path.
    const policyLabel = policyName ? `the ${policyName} preset` : `the policy in ${paths.policyFile}`;
    ...
    // The floor is the policy's own number; a kept file without it cannot be
    // judged, and the signer refuses to boot on the missing key anyway.
    const floor = Number(policyDoc?.rate?.minSignerLamports);
    if (!Number.isFinite(floor)) {
      report.cross('signer SOL', `${policyLabel} has no number for rate.minSignerLamports, so the SOL floor cannot be read`, { fix: `set rate.minSignerLamports in ${paths.policyFile} (the signer refuses to boot on a missing key), or move the file aside and run init again` });
      return done(EXIT_INIT.CROSS);
    }
    ...
    // 5. The policy. The one number init changes in any document, preset or
    // kept file, is the notice, and only when the chain says otherwise; the
    // rewrite is said on its own line.
    const policy = JSON.parse(JSON.stringify(policyDoc));
    const presetNotice = Number(policy.invariants?.rebalanceDelaySecs);
    const noticeRewritten = presetNotice !== notice;
    if (noticeRewritten) {
      policy.invariants = { ...(policy.invariants ?? {}), rebalanceDelaySecs: notice };
      const expects = Number.isFinite(presetNotice) ? `expects a ${presetNotice}s notice` : 'has no number for invariants.rebalanceDelaySecs';
      report.info('policy notice', `${policyLabel} ${expects}; ${symbol} announces ${notice}s, so invariants.rebalanceDelaySecs is written as ${notice}`, { presetNotice: Number.isFinite(presetNotice) ? presetNotice : null, notice });
    }
    const alternatives = Object.fromEntries(PRESETS.filter((n) => n !== policyName).map((n) => [n, loadPreset(n, { repoRoot, fs })]));
    const items = validatePolicyAgainstBook(policy, row, pools, { presetName: policyLabel, alternatives });
    if (items.length) {
      report.cross('policy', `${policyLabel} refuses ${symbol} as it stands (${items.length} item${items.length === 1 ? '' : 's'})`, {
    ...
    if (keepFile) {
      // Kept: the file is written back only when its notice was rewritten,
      // and then with only that value changed.
      if (noticeRewritten) fs.writeFileSync(paths.policyFile, `${JSON.stringify(policy, null, 2)}\n`);
      const how = policyName
        ? `the ${policyName} preset (remembered from ${paths.policyFile}; pass --policy to change it)`
        : `${paths.policyFile} (matches no shipped preset: edited by hand; pass --policy <preset> --yes to write a preset over it)`;
      report.ok('policy', `${how} admits ${legsText(legs)}; kept${noticeRewritten ? ', with only its notice rewritten from chain' : ' as it is'}`, { preset: policyName, presetFrom: policyFrom, kept: true, noticeRewritten });
    } else {
      // Written: what the file held is said on its own line when it was
      // another shipped preset (a switch asked for with --policy) or the
      // owner's edits (written over on --yes only, refused above otherwise).
      if (previousPolicy.present && previousPolicy.preset === null) {
        const was = previousPolicy.doc === null ? 'was not a JSON document' : 'matched no shipped preset (edited by hand)';
        report.info('policy file', `${paths.policyFile} ${was}; the ${policyName} preset is written over it (--policy ${policyName} --yes); put your edits back and restart the signer`, { previousPreset: null });
      ...
      report.ok('policy', `the ${policyName} preset admits ${legsText(legs)}; written to ${paths.policyFile}`, { preset: policyName, presetFrom: policyFrom, kept: false });
    }

    // 6. Tokens and env. The token files are the source of truth: both env
    // files are written from them, never the other way round. A token file
    // that is present wins over any copy (a rotation is a new value in the
    // file, then init), and a copy that differed is named on its own line
    // once both env files are written. A token file that is missing while a
    // copy still holds a value is a cross: a token is never adopted from an
    // env copy, and a fresh one is generated only when no copy holds one.
    const previousSignerEnv = readEnvFile(paths.signerEnv, fs).values;
    const previousAgent = readEnvFile(paths.agentEnv, fs).values;
    const copiesOf = {
      [paths.signerToken]: [['CURATOR_SIGNER_TOKEN', paths.signerEnv, previousSignerEnv.CURATOR_SIGNER_TOKEN], ['CURATOR_SIGNER_TOKEN', paths.agentEnv, previousAgent.CURATOR_SIGNER_TOKEN]],
      [paths.opsToken]: [['CURATOR_OPS_TOKEN', paths.signerEnv, previousSignerEnv.CURATOR_OPS_TOKEN]],
    };
    const tokenOf = (file) => {
      const copies = copiesOf[file].map(([label, envFile, value]) => [label, envFile, String(value ?? '').trim()]).filter(([, , value]) => value !== '');
      if (exists(file, fs)) {
        const r = readTokenFile(file, fs);
        if (!r.ok) throw Object.assign(new Error(r.reason), { step: 'tokens', fix: 'a 64-hex-char token at mode 0600 with no whitespace; to rotate, write the new value into the file and run init again' });
        const differed = copies.filter(([, , value]) => value !== r.token).map(([label, envFile]) => `${label} in ${envFile}`);
        return { token: r.token, generated: false, differed };
      }
      if (copies.length) {
        const named = copies.map(([label, envFile]) => `${label} in ${envFile}`);
        throw Object.assign(new Error(`${file} is missing, but ${named.join(' and ')} still hold${named.length === 1 ? 's' : ''} a value; a token is never adopted from an env copy`), {
          step: 'tokens',
          fix: `to rotate, write the new value into ${file} (mode 0600, no newline) and run init again, which rewrites the copies from it; to have init generate one, first delete the ${copies[0][0]}= line${named.length === 1 ? '' : 's'} from ${named.length === 1 ? copies[0][1] : 'both env files'}`,
        });
      }
      const token = generateToken();
      writeNewSecret(file, token, fs);
      return { token, generated: true, differed: [] };
    };
    ...
    const refreshed = [...signerTok.differed, ...opsTok.differed];
    ...
    const composeValues = {
      ...
      COMPOSE_PROJECT_NAME: composeProjectName(symbol),
      CURATOR_SIGNER_PLATFORM: 'linux/amd64',
    };
    for (const k of ['CURATOR_SIGNER_IMAGE', 'HERMES_IMAGE', 'CURATOR_START_PAUSED', 'CURATOR_TICK_MS', 'COMPOSE_PROJECT_NAME', 'CURATOR_SIGNER_PLATFORM']) {
    ...
    // Said only now: both env files carry the token files' values.
    if (refreshed.length) report.info('tokens', `${refreshed.join(' and ')} differed from the token file; refreshed from the files (restart both services to pick it up)`, { refreshed });
    ...
    report.cross(e.step ?? 'init', e.step ? text : text.slice(0, 300), { fix: e.fix ?? 'fix the cause above and run init again; it reuses what it already wrote' });
    return done(EXIT_INIT.CROSS);
  }
}
 ...
 * `doc` (null when it is not a JSON object) and the shipped `preset` it was
 * written from (null when it matches none apart from the notice).
 */
function readPolicyFile(file, { repoRoot, fs }) {
  ...
  if (!doc || typeof doc !== 'object' || Array.isArray(doc)) return { present: true, doc: null, preset: null };
