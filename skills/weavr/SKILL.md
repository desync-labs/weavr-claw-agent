...
5b. Withdrawals are in whole shares, never USD. If the user gives a dollar amount, take the portfolio price from get_portfolio, divide, round down, and confirm the number of shares with them first. Then run `node $WEAVR_SIGN_TOOL --withdraw <ticker> --shares <n>`. It builds, signs and sends the request; report what it prints. The request joins the exit queue and is paid in order (list_withdrawals shows its place; legs on another chain take longer). Do not call build_withdraw or send_signed yourself.
...
- `--withdraw <ticker> --shares <whole shares>` → builds, signs and sends a withdrawal request; prints `{"status":"confirmed",...}` or an error.
